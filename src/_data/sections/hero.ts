...
/**
 * Load hero section data from JSON file with validation
 * @returns Validated hero section data for the page
 */
export default function(): HeroSection {
  const heroPath = resolve(process.cwd(), 'content/pt-PT/sections/hero.json');
  const rawData = JSON.parse(readFileSync(heroPath, 'utf-8'));

  // Validate the data at build time - fails fast if content is invalid
  if (!validateSectionBase(rawData)) {
    throw new Error(`Invalid hero section base properties: ${JSON.stringify(rawData, null, 2)}`);
  }

  if (rawData.id !== 'hero') {
    throw new Error(`Expected hero section, got: ${rawData.id}`);
  }

  return rawData as HeroSection;
}