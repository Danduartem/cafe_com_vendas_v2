...
export const Footer: Component = {
  ...
  enhanceWhatsAppButton(): void {
    ...
    whatsappBtn.addEventListener('click', function(this: HTMLElement, e) {
      ...
      const whatsappPayload = normalizeEventPayload({
        ...
        link_url: (this as HTMLAnchorElement).href || '', // Will be normalized
  ...
  createParticleEffect(element: HTMLElement) {
    for (let i = 0; i < 6; i++) {
  ...
  enhanceEmailLink(): void {
    const emailLink = safeQuery('#footer a[href^="mailto:"]');
    if (!emailLink) return;

    emailLink.addEventListener('click', function(this: HTMLElement) {
