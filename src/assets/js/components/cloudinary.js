...
export const CloudinaryComponent = {
    ...
    updateCSSBackgrounds(cloudName, imageId) {
        // Create dynamic CSS for responsive backgrounds
        const css = `
            @media (max-width: 640px) {
                .hero-bg[data-cloudinary-bg="${imageId}"] {
                    background-image: url('https://res.cloudinary.com/${cloudName}/image/upload/w_640,h_480,c_fill,q_auto,f_auto,g_auto/${imageId}');
                }
            }
            
            @media (min-width: 641px) and (max-width: 1024px) {
                .hero-bg[data-cloudinary-bg="${imageId}"] {
                    background-image: url('https://res.cloudinary.com/${cloudName}/image/upload/w_1024,h_600,c_fill,q_auto,f_auto,g_auto/${imageId}');
                }
            }
            
            @media (min-width: 1025px) {
                .hero-bg[data-cloudinary-bg="${imageId}"] {
                    background-image: url('https://res.cloudinary.com/${cloudName}/image/upload/w_1920,h_1080,c_fill,q_auto,f_auto,g_auto/${imageId}');
                }
            }
        `;
