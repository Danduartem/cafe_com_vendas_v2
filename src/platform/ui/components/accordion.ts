...
export const PlatformAccordion = {
  ...
  setupRevealAnimation(items: Element[]): void {
    PlatformAnimations.prepareRevealElements(items, {
    ...
    this.initializeNative({
      ...
      onToggle: (item, isOpen) => {
        if (config.trackAnalytics) {
          ...
          import('./analytics.js').then(({ PlatformAnalytics }) => {
            PlatformAnalytics.trackFAQToggle(itemNumber, isOpen, questionText);
          }).catch(() => {
