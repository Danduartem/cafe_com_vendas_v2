...
import type { LoadedPageSection } from '../../../src/_data/types.js';
  ...
  describe('Content Rendering Simulation', () => {
    test('should create valid HTML structure for each section', async () => {
      ...
      enabledSections.forEach((section: LoadedPageSection) => {
        ...
        content.textContent = copy.headline || (copy as { title?: string }).title || 'Content';
    ...
    test('should handle section-specific content correctly', async () => {
      ...
      enabledSections.forEach((section: LoadedPageSection) => {
        ...
        switch (section.slug) {
        case 'hero': {
          const copy = sectionData.copy as { badge?: { date: string; location: string } };
        ...
        case 'problem': {
          const copy = sectionData.copy as { pain_points?: string[] };
