...
describe('GTM Proxy Function', () => {
  ...
  describe('URL Building', () => {
    ...
    it('should always use the same server URL regardless of mode', () => {
      ...
      const url = buildTargetUrl(request);
      expect(url).toBe('https://server-side-tagging-m5scdmswwq-uc.a.run.app/g/collect?v=2&tid=G-T06WRKJGRW&gtm_debug=1');
    });
  });
  ...
  describe('Request Proxying', () => {
    ...
    it('should proxy GET request to production server', async () => {
      const request = new Request('https://jucanamaximiliano.com.br/.netlify/functions/gtm-proxy/g/collect?v=2&tid=G-T06WRKJGRW&cid=test-client', {
        method: 'GET',
        headers: {
          'user-agent': 'Test Browser'
  ...
  describe('Preview Mode Logging', () => {
    it('should log preview mode detection', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('[GTM Proxy] Forwarding request'),
        expect.objectContaining({
          mode: 'preview'
        })
      );
